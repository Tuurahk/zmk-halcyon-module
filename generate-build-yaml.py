        ...
        for bat in data["battery_modules"]:
            if kb["split"] == "true":
                ...
                for mod in data["modules"]:
                    ...
                    # E-paper display builds
                    # Forest is always used on the left half.
                    # Mountain is always used on the right half.
                    if mod["type"] == "epaper_display":
                        if mod["name"] == "mod_display_epaper_forest":
                            # Left side: Forest
                            add_build(builds, c["name"], s_left_base + ["mod_display_epaper_forest"], art_base.format("left"), mode="peripheral")

                            # Left side for Studio usage
                            if BUILD_LEFT_STUDIO:
                                add_build(builds, c["name"], s_left_base + ["mod_display_epaper_forest", "mod_cirque_central"], art_base.format("left") + "_central", mode="studio")

                            # Right side: Mountain
                            add_build(builds, c["name"], s_right_base + ["mod_display_epaper_mountain"], art_base.format("right"))
                        continue
                    
                    # Logic specific to module type
                    elif mod["type"] == "encoder":
